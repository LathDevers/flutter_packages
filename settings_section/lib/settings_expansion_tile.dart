...
/// Enables control over a single [SettingsExpansionTile]'s expanded/collapsed state.
///
/// It can be useful to expand or collapse an [SettingsExpansionTile]
/// programatically, for example to reconfigure an existing expansion
/// tile based on a system event. To do so, create an [SettingsExpansionTile]
/// with an [SettingsExpansionTileController] that's owned by a stateful widget
/// or look up the tile's automatically created [SettingsExpansionTileController]
/// with [SettingsExpansionTileController.of]
///
/// The controller's [expand] and [collapse] methods cause the
/// the [SettingsExpansionTile] to rebuild, so they may not be called from
/// a build method.
class SettingsExpansionTileController {
  /// Create a controller to be used with [SettingsExpansionTile.controller].
  SettingsExpansionTileController();

  SettingsExpansionTileState? _state;

  /// Whether the [SettingsExpansionTile] built with this controller is in expanded state.
  ///
  ...
  /// See also:
  ///
  ///  * [expand], which expands the [SettingsExpansionTile].
  ///  * [collapse], which collapses the [SettingsExpansionTile].
  ///  * [SettingsExpansionTile.controller] to create an ExpansionTile with a controller.
  bool get isExpanded {
    assert(_state != null);
    return _state!._isExpanded;
  }

  /// Expands the [SettingsExpansionTile] that was built with this controller;
  ///
  ...
  ///
  /// Calling this method may cause the [SettingsExpansionTile] to rebuild, so it may
  /// not be called from a build method.
  ///
  /// Calling this method will trigger an [SettingsExpansionTile.onExpansionChanged] callback.
  ///
  /// See also:
  ///
  ...
  void expand() {
    assert(_state != null);
    if (!isExpanded) _state!._toggleExpansion();
  }

  /// Collapses the [SettingsExpansionTile] that was built with this controller.
  ///
  ...
  /// Finds the [SettingsExpansionTileController] for the closest [SettingsExpansionTile] instance
  /// that encloses the given context.
  ///
  /// If no [SettingsExpansionTile] encloses the given context, calling this
  /// method will cause an assert in debug mode, and throw an
  /// exception in release mode.
  ///
  /// To return null if there is no [SettingsExpansionTile] use [maybeOf] instead.
  ///
  /// {@tool dartpad}
  /// Typical usage of the [SettingsExpansionTileController.of] function is to call it from within the
  /// `build` method of a descendant of an [SettingsExpansionTile].
  ///
  /// When the [SettingsExpansionTile] is actually created in the same `build`
  ...
  /// the [SettingsExpansionTileController] (since it's "above" the widget
  ...
  /// can obtain the [SettingsExpansionTileController]. With this approach you
  /// would have an outer widget that creates the [SettingsExpansionTile]
  /// populated by instances of your new inner widgets, and then in
  /// these inner widgets you would use [SettingsExpansionTileController.of].
  static SettingsExpansionTileController of(BuildContext context) {
    final SettingsExpansionTileState? result = context.findAncestorStateOfType<SettingsExpansionTileState>();
    if (result != null) {
      return result.tileController;
    }
  ...
  /// Finds the [SettingsExpansionTile] from the closest instance of this class that
  /// encloses the given context and returns its [SettingsExpansionTileController].
  ///
  /// If no [SettingsExpansionTile] encloses the given context then return null.
  /// To throw an exception instead, use [of] instead of this function.
  ///
  /// See also:
  ///
  ///  * [of], a similar function to this one that throws if no [SettingsExpansionTile]
  ///    encloses the given context. Also includes some sample code in its
  ///    documentation.
  static SettingsExpansionTileController? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<SettingsExpansionTileState>()?.tileController;
  }
}
...
/// [SettingsExpansionTile] to save and restore its expanded state when it is scrolled
/// in and out of view.
///
...
/// to the [leading] and [trailing] properties of [SettingsExpansionTile].
///
/// {@tool dartpad}
/// This example demonstrates how the [SettingsExpansionTile] icon's location and appearance
/// can be customized.
///
...
///
/// {@tool dartpad}
/// This example demonstrates how an [SettingsExpansionTileController] can be used to
/// programatically expand or collapse an [SettingsExpansionTile].
///
/// ** See code in examples/api/lib/material/expansion_tile/expansion_tile.1.dart **
/// {@end-tool}
///
...
class SettingsExpansionTile extends StatefulWidget {
  ...
  /// by a widget within the tile, [SettingsExpansionTileController.of] may be more convenient
  /// than supplying a controller.
  final SettingsExpansionTileController? controller;

  final bool supressExpansion;

  @override
  State<SettingsExpansionTile> createState() => SettingsExpansionTileState();
}

class SettingsExpansionTileState extends State<SettingsExpansionTile> with SingleTickerProviderStateMixin {
  static final Animatable<double> _easeOutTween = CurveTween(curve: Curves.easeOut);
  static final Animatable<double> _easeInTween = CurveTween(curve: Curves.easeIn);
  static final Animatable<double> _halfTween = Tween<double>(begin: -.25, end: .0);
  ...
  late SettingsExpansionTileController tileController;
  ...
  void initState() {
    ...
    tileController = widget.controller ?? SettingsExpansionTileController();
    tileController._state = this;
  }

  @override
  void dispose() {
    tileController._state = null;
  ...
  Widget _buildChildren(BuildContext context, Widget? child) {
    ...
    final EdgeInsetsGeometry padding = widget.tilePadding ?? expansionTileTheme.tilePadding ?? const EdgeInsets.symmetric(horizontal: 16.0);
    final Widget? leading = widget.leading ?? _buildLeadingIcon(context);
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          InkWell(
            onTap: widget.supressExpansion ? null : _handleTap,
                  ...
                  Flexible(
                    child: Row(
                      ...
                      children: [
                        ...
                        if (!widget.supressExpansion)
                          IconTheme.merge(
                            data: IconThemeData(
                              color: _iconColor.value ?? expansionTileTheme.iconColor,
                            ),
                            child: widget.trailing?.call(_isExpanded) ?? _buildTrailingIcon(context) ?? Container(),
                          ),
                      ],
                    ),
                  )
...
class _ExpansionTileDefaultsM2 extends ExpansionTileThemeData {
  ...
  @override
  Color? get iconColor => _theme.colorScheme.primary;

  @override
  Color? get collapsedTextColor => _theme.textTheme.titleMedium!.color;

  @override
  Color? get collapsedIconColor => _theme.colorScheme.primary;
}
