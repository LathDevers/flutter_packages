...
class AutoSizeText extends StatefulWidget {
  ...
  final void Function()? onNotOverflow;
...
class AutoSizeTextState extends State<AutoSizeText> {
  ...
  List<dynamic> _calculateFontSize(BoxConstraints size, TextStyle? style, int? maxLines) {
    ...
    double userScale(double value) {
      if (widget.textScaleFactor != null) return value * widget.textScaleFactor!;
      return MediaQuery.textScalerOf(context).scale(value);
    }
    ...
    if (presetFontSizes == null) {
      final double defaultFontSize = style!.fontSize!.clamp(widget.minFontSize, widget.maxFontSize);
      final double defaultScale = userScale(defaultFontSize) / style.fontSize!;
      if (_checkTextFits(span, defaultScale, maxLines, size)) {
        return <Object>[userScale(defaultFontSize), true];
      }
    ...
    while (left <= right) {
      final int mid = (left + (right - left) / 2).floor();
      double scale;
      if (presetFontSizes == null) {
        scale = userScale(mid.toDouble()) * widget.stepGranularity / style!.fontSize!;
      } else {
        scale = userScale(presetFontSizes[mid]) / style!.fontSize!;
      }
    ...
    double fontSize;
    if (presetFontSizes == null) {
      fontSize = userScale(right.toDouble()) * widget.stepGranularity;
    } else {
      fontSize = userScale(presetFontSizes[right]);
    }
